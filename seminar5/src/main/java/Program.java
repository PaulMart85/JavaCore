...
public class Program {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Tree.print(new File("../seminar5"), "", true);
        ...
        String reserve = backupCopy(new File("../seminar5"));
        if (reserve != null)
            System.out.printf("Создана резервная копия: %s\n", reserve);
    ...
    private static String generateSymbols(int amount){
        ...
        for (int i = 0; i < amount; i++)
            stringBuilder.append((char)(CHAR_BOUND_L+random.nextInt(CHAR_BOUND_H-CHAR_BOUND_L+1)));
        return stringBuilder.toString();
    }

    /**
     * Создает резервную копию всех файлов в директории file (без поддиректорий) во вновь созданную папку ./backup
     * @param file директория, копия которой создается
     * @return String строку с указанием абсолютного пути к резервной копии, иначе null
     */
    private static String backupCopy(File file) throws IOException {

        if (!file.exists()) return null; // источник указан неверно

        File[] files = file.listFiles(f -> f.isFile()); // список файлов в директории file (без поддиректорий)
        if (files == null) // нечего копировать
            return null;

        Path dest = Paths.get("../seminar5/backup");
        dest.toFile().mkdir();

        for (File fil : files)
            Files.copy(fil.toPath(), dest.resolve(fil.getName()), REPLACE_EXISTING);
